...
function exec<?php echo $widget->getDomId(); ?>() {
  tbApp.onScriptLoaded(function() {
    ...
    var $slider = new mightySlider(
      '#<?php echo $widget->getDomId(); ?> .frame',
      {
        speed:             500,
        easing:            'easeOutExpo',
        viewport:          'fit',
    ...
    $slider.one('coverLoaded', function (eventName) {
      $('#<?php echo $widget->getDomId(); ?> .tb_thumbs ul').removeClass('tb_grid_view');
    });
    ...
    <?php // Fullscreen gallery ?>

    <?php if ($fullscreen && $gallery_type == 'slide'): ?>
...
<?php if ($tbData->optimize_js_load && $gallery_type == 'grid'): ?>
