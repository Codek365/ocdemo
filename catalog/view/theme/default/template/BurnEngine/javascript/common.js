            ...
            success: function (json) {
                ...
                if (json['success']) {
                    ...
                    $.get('index.php?route=common/cart/info', function(result) {
                        ...
                        $('.tb_widget_header_cart_menu_system .heading').replaceWith($title);
                        $('.tb_widget_header_cart_menu_system .content').replaceWith($items);
        ...
        $.ajax({
          ...
          success: function(json) {
            ...
            if (tbApp['/tb/route'] == 'checkout/cart' || tbApp['/tb/route'] == 'checkout/checkout') {
              ...
              $('.tb_widget_header_cart_menu_system').load('index.php?route=common/cart/info');
            }
          }
        });
    },
...
var tb_user_agent   = navigator.userAgent,
    tb_is_iphone    = ~tb_user_agent.indexOf('iPhone') || ~tb_user_agent.indexOf('iPod'),
    tb_is_ipad      = ~tb_user_agent.indexOf('iPad'),
    tb_is_ios       = tb_is_iphone || tb_is_ipad,
    tb_is_android_5 = ~tb_user_agent.indexOf('Android 5'),
...
function displayNotice(context, type, product_id, message) {
    ...
    if (!$product.length) {
        $product = $('.tb_widget_product_images_system');
    ...
    var buttons_config = [{
        type: 'btn', text: tbApp['/lang/text_continue'], click: function() {
            $.noty.closeAll();
        }
    }];
    ...
    if (type == 'failure') {
        showNotice(tbApp['/lang/text_failure']);
        return;
    }
    ...
    switch (context) {
        case 'product':
            buttons_config = [{
                type: 'btn', text: tbApp['/lang/text_continue_shopping'], click: function() {
                    $.noty.closeAll();
                }
            },{
                type: 'btn', text: tbApp['/lang/text_shopping_cart'], click: function() {
                    window.location = tbApp['/tb/url/shopping_cart'];
                }
            }];

            showNotice(tbApp['/lang/text_cart_updated']);
            break;
        case 'wishlist':
            buttons_config.push({
                type: 'btn', text: tbApp['/lang/text_wishlist'], click: function() {
                    window.location = tbApp['/tb/url/wishlist'];
                }
            });
            showNotice(tbApp['/lang/text_wishlist_updated']);
            break;
        case 'compare':
            buttons_config.push({
                type: 'btn', text: tbApp['/lang/text_product_comparison'], click: function() {
                    window.location = tbApp['/tb/url/compare'];
                }
            });
            showNotice(tbApp['/lang/text_compare_updated']);
            break;
    }
}
...
function sticky_header(style, layout, padding) {
    ...
    tbApp.onWindowLoaded(function() {
        ...
        $(window).scroll(function () {
            if (window.pageYOffset > height + offset.top - 20 && tbApp.windowWidth > 766 && !tb_navigation_hovered) {
                if ($wrap.find('> .tbSticky').length == 0) {
                    ...
                    if (style == 'minimal' && $('#sticky_header').length == 0) {
                        $temp = $('<div id="sticky_header" class="tb_area_header ' + classes + ' tbSticky" style="padding: ' + padding + '"><div class="row tbStickyRow"><div class="col-xs-12 col-md-12 col-valign-middle"></div></div></div>').prependTo($wrap);
                        ...
                        $temp.find('.tb_megamenu > .dropdown-menu').each(function () {
                            $(this).attr("id", 'cloned_dropdown_' + String(Math.random()).replace(".", ""));
                            adjustItemSize("#" + $(this).attr("id") + ' .tb_subcategories');
                        });
                        $temp.find('.tb_megamenu.tb_tabbed_menu').each(function () {
                            $(this).find('[data-target]').each(function () {
                                var target = $(this).data('target');

                                $(this).data('target', target + '_cloned');
                            });
                            $(this).find('.tab-content > li').each(function () {
                                var id = $(this).attr('id');

                                $(this).attr('id', id + '_cloned');
                            });
                            tabbed_menu($(this));
                        });
                    }
    ...
    $('#header')
        .on('click', '.tbToggleButtons span', function () {
            if ($(this).hasClass('tbToggleCart')) {
                $temp = $('<div class="tbMobileMenu tbMobileCartMenu"><div class="row"><div class="col-xs-12 col-sm-12 col-valign-top"></div></div></div>').appendTo('#wrapper');

                $('#header').find('.tb_widget_header_cart_menu_system').each(function () {
                ...
                $temp = $('<div class="tbMobileMenu"><div class="row"><div class="col-xs-12 col-sm-12 col-valign-top"></div></div></div>').appendTo('#wrapper');
...
function createItemSlider(container, items_count, slide_step, speed, pagination, responsive_params, autoplay, loop) {
    ...
    var $heading        = $(container).is('.has_slider:not(.no_title)') ? $(container).find('.panel-heading, .box-heading') : $(container).closest('.tb_widget_group').find('.nav-tabs'),
    ...
    function createSlider(slides_per_view, slide_step) {
        ...
        $(container)
            ...
            .after('<div class="tb_slider_controls">' +
                   '  <a class="tb_prev" href="javascript:;" title="' + tbApp['/lang/text_previous'] + '">' +
                   '    <svg><use xlink:href="' + window.location.href + '#chevron_thin" /></svg>' +
                   '  </a>' +
                   '  <a class="tb_next" href="javascript:;" title="' + tbApp['/lang/text_next'] + '">' +
                   '    <svg><use xlink:href="' + window.location.href + '#chevron_thin" /></svg>' +
                   '  </a>' +
                   '</div>');
    ...
    function responsive() {
        ...
        var $width_container = $(container).find('.tb_side_nav').length ? $(container).find('.tb_side_nav') : $(container),
        ...
        if ($heading.length < 1) {
            $(container).find('.tb_slider').addClass('tb_side_nav');
        }
        ...
        if ($(container).find('.tb_slider').hasClass('tb_side_nav')) {
            ...
            var $container = $(container).is('.has_slider:not(.no_title)') ? $(container) : $(container).closest('.tb_widget_group'),
        ...
        if (tb_is_android_5) {
            $('body').append('<style id="chrome_fix" type="text/css">' + container + ' { white-space: nowrap; }</style>');
            setTimeout(function () {
                $('#chrome_fix').remove();
            }, 10);
        }
...
function cookie_policy(policy_text) {
    ...
    noty({
        text: '<h3>' + tbApp['/lang/text_cookie_policy_title'] + '</h3><p>' + policy_text + '</p>',
        layout: tbApp['/tb/msg_position'],
        closeOnSelfClick: false,
        modal: false,
        buttons: [{
            type: 'btn',
            text: tbApp['/lang/text_cookie_policy_button'],
            click: function() {
...
function tabbed_menu($menu) {
    ...
    $nav
        .on('mouseenter', $menu, function() {
            if ($nav.hasClass('nav-justified-dropdown')) {
                $(this).find('.nav-tabs').css('min-width', $menu.width());
            }
            $menu.find('.nav-tabs > *').removeClass('active');
            $menu.find('.tab-content > *').hide();
            $menu.find('.nav-tabs > :first-child').addClass('active');
            $($menu.find('.nav-tabs > :first-child [data-target]').data('target')).show();
...
function bootApp() {
    ...
    element_query('.tb_widget_product_options_system', '1000,300,0');
    element_query('.tb_widget_product_add_to_cart_system', '400,0');
    element_query('.tb_system_search_box', '550,0');

    $body
        .on(tb_is_ios ? 'touchend' : 'click', '.dropdown > .tb_toggle, .dropdown > a[href*="javascript"]', function() {
            if (!$(this).parent().find('> .dropdown-menu, > .hidden + ul').is(':visible')) {
        ...
        .hoverIntent({
            over: function(){
                ...
                if ($nav.data('relative_to') == 'content' && orientation == 'horizontal' && $relativeTo.width() < current_site_width) {
                    ...
                    menu_width += orientation == 'vertical' ? -$nav.width() : 0;
                }
    ...
    $('#wrapper')
        .on('click', '.dropdown > a', function(e) {
            if (tbApp["/tb/is_touch"] && !$(this).next('.dropdown-menu').is(':visible') && !$(this).parent().find('> .tb_toggle').is(':visible')) {
                e.preventDefault();
            }
        })
