...
function createItemSlider(container, items_count, slide_step, speed, pagination, responsive_params, autoplay) {
...
tbApp.onScriptLoaded(function() {
    ...
    $("body").on("click", ".agree, .colorbox, .fancybox, .tb_popup", function() {
        tbCreateDialog($(this));
    });
    ...
    $('#wrapper').hoverIntent({
        over: function(){
            ...
            var $dropdown = $(this).find('.tb_submenu'),
                scroll = $(window).scrollTop(),
                height = $dropdown.outerHeight(),
                offset = $dropdown.offset();

            if (window.innerHeight < height) {
                $dropdown.css('max-height', (window.innerHeight - offset.top + scroll - 60));
        ...
        selector: '.tb_widget_cart > .tb_nav',
        timeout: 250
    });
