...
$(this).siblings('ul.dropdown-menu').html(html);};$(this).after('<ul class="autocomplete-menu dropdown-menu"></ul>');$(this).siblings('ul.dropdown-menu').delegate('a','click',$.proxy(this.click,this));});}})(window.jQuery);tbApp.onScriptLoaded(function(){tbApp.windowWidth=window.innerWidth;tbUtils.onSizeChange(function(){tbApp.windowWidth=window.innerWidth;},false,false,'windowWidth');includeSvgResource("catalog/view/theme/"+tbApp["/tb/basename"]+"/image/icons.svg",2);});var tb_user_agent=navigator.userAgent,tb_is_iphone=~tb_user_agent.indexOf('iPhone')||~tb_user_agent.indexOf('iPod'),tb_is_ipad=~tb_user_agent.indexOf('iPad'),tb_is_ios=tb_is_iphone||tb_is_ipad,tb_is_android_5=~tb_user_agent.indexOf('Android 5'),tb_navigation_hovered=false;function displayNotice(context,type,product_id,message){var stack=Number(tbApp['/tb/msg_stack']);var thumb;if(!stack){$.noty.closeAll();}
var $product=$(".product-id_"+product_id).first().parent();if(!$product.length){$product=$('.tb_widget_product_images_system');thumb=!$product.find('.image img').length?$product.find('.thumbnail img').attr('src'):$product.find('.image img').attr('src');}else{thumb=$product.find(".image img").attr("src");}
if(!thumb){thumb=tbApp['/tb/no_image'];}
var buttons_config=[{type:'btn',text:tbApp['/lang/text_continue'],click:function(){$.noty.closeAll();}}];var modal=false;var w_width=window.innerWidth;var w_height=window.innerHeight;if(w_width<767&&w_height<767){modal=true;}
...
var showNotice=function(title){var template_vars={icon:type=='success'?'check':'close',title:title,contents:message,thumb_src:thumb,url:window.location.href};noty({text:$.tim(msg_tpl,template_vars),layout:tbApp['/tb/msg_position'],closeOnSelfClick:false,modal:modal,buttons:modal?buttons_config:false,closeButton:!modal,timeout:modal?false:Number(tbApp['/tb/msg_timeout']),animateOpen:{opacity:'toggle'},animateClose:{opacity:'toggle'},close_speed:stack?500:0,onClose:function(){$(document).unbind('touchmove.noty');}});};if(type=='failure'){showNotice(tbApp['/lang/text_failure']);return;}
switch(context){case'product':buttons_config=[{type:'btn',text:tbApp['/lang/text_continue_shopping'],click:function(){$.noty.closeAll();}},{type:'btn',text:tbApp['/lang/text_shopping_cart'],click:function(){window.location=tbApp['/tb/url/shopping_cart'];}}];showNotice(tbApp['/lang/text_cart_updated']);break;case'wishlist':buttons_config.push({type:'btn',text:tbApp['/lang/text_wishlist'],click:function(){window.location=tbApp['/tb/url/wishlist'];}});showNotice(tbApp['/lang/text_wishlist_updated']);break;case'compare':buttons_config.push({type:'btn',text:tbApp['/lang/text_product_comparison'],click:function(){window.location=tbApp['/tb/url/compare'];}});showNotice(tbApp['/lang/text_compare_updated']);break;}}
...
function scroll_to_top(speed){if(window.innerWidth<=767&&window.innerHeight<=767){return;}
...
function sticky_header(style,layout,padding){if($('#header').length==0||window.innerWidth<=767&&window.innerHeight<=767){return;}
if(style===undefined){style='minimal';}
if(layout===undefined){layout='full';}
if(padding===undefined){padding='10px 0';}
var $cont=$('#header'),$wrap=$('#wrapper'),$temp;var height=$cont.outerHeight(true),offset=$cont.offset(),old_classes=$cont.attr('class'),classes=layout!='full_fixed'?'tb_sticky_width_'+layout:'tb_sticky_width_full tb_content_fixed';tbApp.onWindowLoaded(function(){$(window).scroll(function(){if(window.pageYOffset>height+offset.top-20&&tbApp.windowWidth>766&&!tb_navigation_hovered){if($wrap.find('> .tbSticky').length==0){function clone_widget($widget){tbApp.trigger('beforeCloneWidget',[$widget]);var $clonedWidget=$widget.clone(true).css({'padding-left':0,'padding-right':0}).removeClass('display-block').addClass('display-inline-block').find("script").remove().end();$temp.find('.col-md-12').append($clonedWidget);}
if(style=='minimal'&&$('#sticky_header').length==0){$temp=$('<div id="sticky_header" class="tb_area_header '+classes+' tbSticky" style="padding: '+padding+'"><div class="row tbStickyRow"><div class="col-xs-12 col-md-12 col-valign-middle"></div></div></div>').prependTo($wrap);$cont.find('.tbStickyShow:not([class*="tbStickyPosition"])').each(function(){clone_widget($(this));});for(var i=1;i<=8;i++){$cont.find('.tbStickyPosition-'+i).each(function(){clone_widget($(this));});}
$temp.find('.tb_megamenu > .dropdown-menu').each(function(){$(this).attr("id",'cloned_dropdown_'+String(Math.random()).replace(".",""));adjustItemSize("#"+$(this).attr("id")+' .tb_subcategories');});$temp.find('.tb_megamenu.tb_tabbed_menu').each(function(){$(this).find('[data-target]').each(function(){var target=$(this).data('target');$(this).data('target',target+'_cloned');});$(this).find('.tab-content > li').each(function(){var id=$(this).attr('id');$(this).attr('id',id+'_cloned');});tabbed_menu($(this));});}
...
function responsive_header(){var $temp;$('#header').on('click','.tbToggleButtons span',function(){if($(this).hasClass('tbToggleCart')){$temp=$('<div class="tbMobileMenu tbMobileCartMenu"><div class="row"><div class="col-xs-12 col-sm-12 col-valign-top"></div></div></div>').appendTo('#wrapper');$('#header').find('.tb_widget_header_cart_menu_system').each(function(){var $clonedMenu=$(this).clone(true).find("script").remove().end();$temp.find('.col-sm-12').append($clonedMenu);$('html').addClass('tbCartMenu');element_query('.tbMobileCartMenu .cart-info','500,300,0');});}else{$temp=$('<div class="tbMobileMenu"><div class="row"><div class="col-xs-12 col-sm-12 col-valign-top"></div></div></div>').appendTo('#wrapper');$('#header').find('.tbMobileMenuShow:not([class*="tbMobilePosition"])').each(function(){var $clonedMenu=$(this).clone(true).find("script").remove().end();$temp.find('.col-sm-12').append($clonedMenu);});for(var i=1;i<=8;i++){$('#header').find('.tbMobileMenuShow.tbMobilePosition-'+i).each(function(){var $clonedMenu=$(this).clone(true).find("script").remove().end();$temp.find('.col-sm-12').append($clonedMenu);});}}
setTimeout(function(){$('.tbMobileMenuOverlay').addClass('tbActive');$('html').addClass('tbMobile');tbApp.triggerResizeCallbacks();},10);});$('#wrapper').append(''+'<div class="tbMobileMenuOverlay">'+'  <svg><use xlink:href="'+window.location.href+'#close" /></svg>'+'  <span class="tb_bg"></span>'+'</div>');$('#wrapper').on('click','> .tbMobileMenuOverlay',function(){$('html').removeClass('tbMobile');$('.tbMobileMenuOverlay').removeClass('tbActive');setTimeout(function(){$('#wrapper').find('> .tbMobileMenu').remove();},500);tbApp.triggerResizeCallbacks();});if(window.innerWidth<=767&&window.innerHeight>767||window.innerWidth>767&&window.innerHeight<=767){function responsive(){if(window.innerWidth>767){$('#wrapper').find('> .tbMobileMenu').remove();$('html').removeClass('tbMobile');$('.tbMobileMenuOverlay').removeClass('tbActive');}}
...
var $heading=$(container).is('.has_slider:not(.no_title)')?$(container).find('.panel-heading, .box-heading'):$(container).closest('.tb_widget_group').find('.nav-tabs'),$slides=$(container+' .tb_listing').children(),init=false,responsive_keys=[];for(var k in responsive_params){responsive_keys.push(Number(k));}
...
$(container).find('.tb_listing').after('<div class="tb_slider_controls">'+'  <a class="tb_prev" href="javascript:;" title="'+tbApp['/lang/text_previous']+'">'+'    <svg><use xlink:href="'+window.location.href+'#chevron_thin" /></svg>'+'  </a>'+'  <a class="tb_next" href="javascript:;" title="'+tbApp['/lang/text_next']+'">'+'    <svg><use xlink:href="'+window.location.href+'#chevron_thin" /></svg>'+'  </a>'+'</div>');SwiperObj=new Swiper(container+' .swiper-container',{slidesPerView:slides_per_view,slidesPerGroup:slide_step,speed:speed,autoplay:autoplay,loop:loop,loopAdditionalSlides:slides_per_view,pagination:pagination,paginationClickable:true,paginationActiveClass:'tb_active',roundLengths:true,autoResize:false,onSwiperCreated:function(swiper){buttons();$(container).find('.swiper-container').addClass('tb_slider_init');},onSlideChangeEnd:function(swiper){buttons();},onTouchEnd:function(swiper){buttons();}});$(container).on('click','.tb_prev, .tb_next',function(){$(container).addClass('tbSliderInteracted');});$(container).on('mouseover',function(){SwiperObj.stopAutoplay();});$(container).on('mouseout',function(){if(!$(this).is('.tbSliderInteracted')){SwiperObj.startAutoplay();}});swiperDeferred.resolve(SwiperObj);}
...
var $nav=$(container).find('.tb_slider_controls');if($heading.length<1){$(container).find('.tb_slider').addClass('tb_side_nav');}
if($(container).find('.tb_slider').hasClass('tb_side_nav')){tbApp.onWindowLoaded(function(){var nav_height=$nav.find('> a:first-child').height(),mod_left=parseInt($(container).css('padding-left')),mod_right=parseInt($(container).css('padding-right'));$nav.find('> a').css('margin-top',-($(container+' .swiper-container').height()/2+nav_height/2));$nav.find('> .tb_prev').css('margin-left',mod_left);$nav.find('> .tb_next').css('margin-right',mod_right);$nav.css('visibility','visible');});}
else{var $container=$(container).is('.has_slider:not(.no_title)')?$(container):$(container).closest('.tb_widget_group'),lang_dir=$('html').first().attr('dir'),mod_x=lang_dir=='ltr'?parseInt($heading.css('padding-right'))
...
if(tb_is_android_5){$('body').append('<style id="chrome_fix" type="text/css">'+container+' { white-space: nowrap; }</style>');setTimeout(function(){$('#chrome_fix').remove();},10);}}
$(container).on('click','.tb_prev',function(e){e.preventDefault();SwiperObj.swipePrev();buttons();});$(container).on('click','.tb_next',function(e){e.preventDefault();SwiperObj.swipeNext();buttons();});return{refresh:function(){if(false!==responsive_params){responsive();if(false===init){tbUtils.onSizeChange(function(){if($(container).is(":visible")){responsive();if($(container).parent().parent().hasClass('tb_tabs_content')){$(container).parent().css('height','');}}},false,false,'createItemSlider_'+container);init=true;}}else
...
noty({text:'<h3>'+tbApp['/lang/text_cookie_policy_title']+'</h3><p>'+policy_text+'</p>',layout:tbApp['/tb/msg_position'],closeOnSelfClick:false,modal:false,buttons:[{type:'btn',text:tbApp['/lang/text_cookie_policy_button'],click:function(){$.cookie('agreed_with_cookie_policy','1');$.noty.closeAll();}}],closeButton:false,timeout:false,animateOpen:{opacity:'toggle'},animateClose:{opacity:'toggle'},close_speed:500,onClose:function(){$(document).unbind('touchmove.noty');}});}
function tabbed_menu($menu){var $nav=$menu.closest('.nav');$nav.on('mouseenter',$menu,function(){if($nav.hasClass('nav-justified-dropdown')){$(this).find('.nav-tabs').css('min-width',$menu.width());}
$menu.find('.nav-tabs > *').removeClass('active');$menu.find('.tab-content > *').hide();$menu.find('.nav-tabs > :first-child').addClass('active');$($menu.find('.nav-tabs > :first-child [data-target]').data('target')).show();$menu.toggleClass('tb_first_tab_selected',$menu.find('.nav-tabs > li:first-child').is('.active'));}).hoverIntent({over:function(){$menu.find('.tab-content > *').hide();$menu.find('.nav-tabs > *').removeClass('active');$(this).parent().addClass('active');$($(this).data('target')).show();$menu.toggleClass('tb_first_tab_selected',$menu.find('.nav-tabs > li:first-child').is('.active'));},out:function(){},selector:'[data-target]',timeout:100});}
function bootApp(){var $body=$('body');var $uiButton=$.fn.button;$.fn.button=function(){if(arguments.length==1&&typeof arguments[0]=='string'){var $button=$(this);var position=$button.parent().hasClass('pull-right')?'before':'after';if(arguments[0]=='loading'){$button.attr('disabled',true);if(position=='before'){$button.prev('.fa-spin').remove();$button.before('<i class="fa fa-circle-o-notch fa-spin"></i>');}else{$button.next('.fa-spin').remove();$button.after('<i class="fa fa-circle-o-notch fa-spin"></i>');}}
if(arguments[0]=='reset'){$button.attr('disabled',false);if(position=='before'){setTimeout(function(){$button.prev('.fa-spin').fadeOut(300,function(){$(this).remove();});},500);}else{setTimeout(function(){$button.next('.fa-spin').fadeOut(300,function(){$(this).remove();});},500);}}}else{$uiButton.apply(this,arguments);return this;}};$('span.error, .text-danger').each(function(){$(this).closest('tr, .form-group').addClass('has-error');});$body.on('click','.alert, .success, .attention, .warning, .information',function(){$(this).fadeTo(200,0,function(){$(this).slideUp(300,function(){$(this).remove();});});});$('#currency, #form-currency').on('click','a[data-currency-code]',function(){changeCurrency($(this));});$('#language, #form-language').on('click','a[data-language-code]',function(){changeLanguage($(this));});$('#search_button').bind('click',function(){moduleSearch($(this));});$('#filter_keyword').bind('keydown',function(e){if(e.keyCode==13){var $element=$(this);tbApp.once('filterKeywordEnter',function(context){if(context.redirect){moduleSearch($element);}});tbApp.trigger('filterKeywordEnter',[{redirect:true}]);}});$('[data-toggle="tooltip"]').tooltip({container:'body'});$(document).ajaxStop(function(){$('[data-toggle="tooltip"]').tooltip({container:'body'});});$body.on('click','.agree, .colorbox, .fancybox, .tb_popup',function(e){e.preventDefault();var $link=$(this);if($link.data('dialog')){$('#'+$link.data('dialog')).modal('show');}else{var dialog_id='tbModal-'+Math.floor(Math.random()*100)+1;html='<div id="'+dialog_id+'" class="modal fade">';html+='  <div class="modal-dialog">';html+='    <div class="modal-content">';html+='      <div class="modal-header">';html+='        <h2 class="modal-title">'+$link.text()+'</h2>';html+='        <a href="javascript:;" class="close" data-dismiss="modal"></a>';html+='      </div>';html+='      <div class="modal-body"></div>';html+='    </div>';html+='  </div>';html+='</div>';$('body').append(html);$('#'+dialog_id).modal('show').on('shown.bs.modal',function(e){if($link.data('dialog')){return;}
var jqxhr=$.ajax({url:$link.attr('href'),type:'get',dataType:'html',beforeSend:function(){$('#'+dialog_id).find('.modal-body').addClass('tb_loading tb_blocked').append('<span class="wait"></span>');}});setTimeout(function(){jqxhr.done(function(data){$('#'+dialog_id).find('.modal-body').html(data).removeClass('tb_loading tb_blocked');});},500);$link.data('dialog',dialog_id);});}});if(tbApp["/tb/is_touch"]!=0){$body.addClass('is_touch');}
element_query('.responsive');element_query('.cart-info','500,300,0');element_query('#cart','500,300,0','.cart-info');element_query('.tb_listing_options','1000,650,350,0');element_query('.tb_widget_product_options_system','1000,300,0');element_query('.tb_widget_product_add_to_cart_system','400,0');element_query('.tb_system_search_box','550,0');$body.on(tb_is_ios?'touchend':'click','.dropdown > .tb_toggle, .dropdown > a[href*="javascript"]',function(){if(!$(this).parent().find('> .dropdown-menu, > .hidden + ul').is(':visible')){$(this).parent().addClass('tb_active').find('> .tb_toggle').addClass('tb_active');}else{$(this).parent().removeClass('tb_active').find('> .tb_toggle').removeClass('tb_active');}
tbApp.triggerResizeCallbacks();}).hoverIntent({over:function(){var tb_sticky_parent;$(this).addClass('tb_hovered');if($(this).closest('#header').length){tb_navigation_hovered=true;}
...
if(!$nav.length||!$menu.hasClass('tb_megamenu')){var menu_space=$dropdown.outerWidth()+$dropdown.offset().left;var margin_left=menu_space>tbApp.windowWidth?tbApp.windowWidth-menu_space-30:0;$dropdown.css('margin-left',margin_left);}
