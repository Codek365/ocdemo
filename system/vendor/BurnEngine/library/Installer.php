...
class TB_Installer
{
    ...
    public function installEngine($type)
    {
        ...
        if ($this->checkInstallRequirements()) {
            ...
            $_COOKIE['can_install_sample_data'] = 1;
            setcookie('can_install_sample_data', 1, time() + 604800, $this->context->getAdminBaseUrl(), $this->engine->getOcRequest()->server['HTTP_HOST']);

            if ($this->engine->gteOc2()) {
                $this->installOc2($type);
            } else {
                $this->installOc1();
            }
        }
    ...
    protected function installOc2($type)
    {
        if ($this->engine->gteOc22()) {
            $opposite_type = $type == 'module' ? 'theme' : 'module';

            /** @var ModelExtensionExtension $ocExtensionModel */
            $ocExtensionModel = $this->engine->getOcModel('extension/extension');
            $ocExtensionModel->install($opposite_type, TB_Engine::getName());

            /** @var ModelUserUserGroup $ocUserGroupModel */
            $ocUserGroupModel = $this->engine->getOcModel('user/user_group');
            $ocUserGroupModel->addPermission($this->engine->getOcUser()->getGroupId(), 'access', $opposite_type . '/' . TB_Engine::getName());
            $ocUserGroupModel->addPermission($this->engine->getOcUser()->getGroupId(), 'modify', $opposite_type . '/' . TB_Engine::getName());
        }
    ...
    protected function installEngineOcMod()
    {
        ...
        if (!$this->engine->gteOc22()) {
            ...
            $extensionEvent->addEvent(TB_Engine::getName(), 'admin/controller/*/before', 'theme/BurnEngine/onControllerBefore');
        }
    ...
    public function checkPavilionUpgrade($burnengine_theme_id)
    {
        foreach ($this->engine->getThemeModel()->getAllStores() as $store) {
            if ($theme_settings = $this->engine->getDbSettingsHelper('setting')->getKey('theme_pavilion', $store['store_id'], 'pavilion')) {
                ...
                unset($_COOKIE['can_install_sample_data']);
                setcookie('can_install_sample_data', null, -1, $this->context->getAreaRelativeUrl(), $this->engine->getOcRequest()->server['HTTP_HOST']);
