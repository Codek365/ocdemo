...
class TB_OutputHandler {
    ...
    protected function buildOptimizedJavascript(&$js, &$items_to_replace, $html)
    {
        ...
        if ($this->themeData->system['image_lazyload'] || $this->themeData->system['bg_lazyload'])  {
            //$inline_js = file_get_contents($this->context->getCatalogResourceDir() . '/javascript/aspectratio.min.js') . "\n" .$inline_js;
        }
    ...
    public function assignJavascriptAfterDispatch()
    {
        ...
        if ($this->themeData->system['image_lazyload'] || $this->themeData->system['bg_lazyload'])  {
            if (!$this->themeData->optimize_js_load) {
                //$this->themeData->prependJavascriptResource('javascript/aspectratio.min.js');
            }
    ...
    protected function buildCssCacheHash()
    {
        ...
        $hash .= '.' . ($this->themeData->is_touch ? 'touch.' : '' . $this->context->getStoreId());
