...
class Theme_Catalog_Extension extends TB_CatalogExtension
{
    ...
    public function link($route, $args = '', $connection = 'NONSSL')
    {
        ...
        if (isset($data['route'])) {
            ...
            if ($data['route'] == 'category/manufacturer' && isset($data['c_id']) && isset($data['man_id'])) {
                ...
                $pos = strcspn($category_url ^ implode('/', $category_parts), "\0");
                $base_url = rtrim(substr_replace($category_url, '', -$pos), '/');

                return $base_url . '/' . implode('/', $category_parts) . '/' . implode('/', $brand_parts);
            }
        }
    ...
    public function setTouch()
    {
        ...
        setcookie('is_touch', $this->themeData->is_touch, time() + 604800, '/', $this->request->server['HTTP_HOST']);
    }
