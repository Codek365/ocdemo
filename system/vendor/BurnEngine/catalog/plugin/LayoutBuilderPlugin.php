...
class Theme_Catalog_LayoutBuilderPlugin extends TB_ExtensionPlugin
{
    ...
    protected function addWidgetAssets(TB_Widget $widget, TB_StyleBuilder $styleBuilder)
    {
        foreach (array('box', 'title') as $style_type) {
            ...
            if (!$widget->getSettings('preset_id') || !empty($styles['colors'])) {
                TB_ColorSchemer::getInstance()->filterWidgetColors($styles['colors'], $style_type == 'box'  ? $widget->getDefaultBoxColors() : $widget->getDefaultTitleColors());

                $this->getModel('layoutBuilder')->cleanSettingsColorsBeforePersist($styles['colors']);
        ...
        if (empty($layout['separate_columns'])) {
            if(isset($layout['columns_gutter']) && strlen($layout['columns_gutter']) && (int) $layout['columns_gutter'] >= 0) {
                $classes .= $layout['columns_gutter'] > 30 ?
                    ' tb_gut_xs_30 tb_gut_sm_30 tb_gut_md_' . $layout['columns_gutter'] :
                    ' tb_gut_xs_'  . $layout['columns_gutter'] . ' tb_gut_sm_'  . $layout['columns_gutter'] . ' tb_gut_md_' . $layout['columns_gutter'] ;
            }
            else {
                $classes .= ' tb_gut_xs_30 tb_gut_sm_30';
            }
        } else {
    ...
    public function buildColumnClasses(array $column_settings, array $row_settings)
    {
        ...
        $proportion = explode('_', $column_settings['grid_proportion']);
        $grid = !in_array($proportion[0], array('auto', 'fill')) ? 12 * $proportion[0] / $proportion[1] : (string) $proportion[0];

        $classes  = ' col-xs-12';
        $classes .= is_int($grid) ? ' col-sm-' . $grid . ' col-md-' . $grid : ' col-sm-' . (is_string($grid) ? $grid : $proportion[0] . '-' . $proportion[1]) . ' col-md-' . (is_string($grid) ? $grid : $proportion[0] . '-' . $proportion[1]);
