...
class Theme_Catalog_MenuPlugin extends TB_ExtensionPlugin
{
    ...
    protected function generateMenuIcon($menu_item)
    {
        ...
        if ($settings['menu_icon_type'] == 'symbol' && !empty($settings['menu_icon'])) {
            ...
            $menu_icon         = $settings['menu_icon'] ? '<span class="tb_icon ' . $settings['menu_icon'] . '"' . $menu_icon_styles . '></span>' : '';
        }
        if ($settings['menu_icon_type'] == 'image' && !empty($settings['menu_icon_image']) && is_file(DIR_IMAGE . $settings['menu_icon_image'])) {
            ...
            $menu_icon = '<img class="tb_icon" src="' . $this->engine->getContext()->getImageUrl() . $settings['menu_icon_image'] . '" alt="" />';
        }
            ...
            switch ($item['data']['type']) {
                ...
                case 'category':
                    if ($category = $this->getCategoryInfo($item)) {
                        $html .= $this->fetchMenuCategory($category, $item, $return);
                    }
                    break;
                case 'categories';
    ...
    public function getCategorySubMenu($category, array $menu_settings, $max_depth = null, $all_categories = false)
    {
    ...
    protected function fetchMenuCategory($category, $menu_item, $return = false)
    {
        $settings  = $menu_item['data']['settings'];
        ...
        $settings['wrapper_classes'] .= $settings['is_megamenu'] ? ($settings['subcategory_direction'] == 'row' ? ' tb_listing tb_grid_view tb_gut_xs_20 tb_gut_sm_20 tb_gut_md_20 tb_style_1' : ' tb_multicolumn') : '';
        ...
        //$category_label = $return != 'menus' ? (isset($settings['label']) && !empty($settings['label'] ) ? $settings['label'] : $category['name']) : false;
        ...
        return $this->extension->fetchTemplate('menu/category', array(
            ...
            'menu_classes'           => !empty($menu_classes) ? ' class="' . $menu_classes . '"' : '',
    ...
    protected function fetchMenuAllCategories($menu_item)
    {
        ...
        $wrapper_classes         = '';
        $wrapper_classes        .= $settings['is_megamenu'] ? ' tb_listing tb_grid_view tb_gut_xs_20 tb_gut_sm_20 tb_gut_md_20 tb_style_1 tb_size_4' : '';
        ...
        foreach($categories as $key => &$category) {
            ...
            $category['submenu'] = $this->getCategorySubMenu($category, $settings, $max_depth, $settings['is_megamenu']);
        }
