...
class Theme_Catalog_ProductPlugin extends TB_ExtensionPlugin
{
    ...
    public function addProductData(array &$products, $settings)
    {
        ...
        foreach ($products as &$product) {
            ...
            $product['show_cart'] = isset($product['quantity']) && ($product['quantity'] > 0 || $this->engine->getOcConfig()->get('config_stock_checkout')) && $themeData->common['checkout_enabled'] && $settings['show_cart'] && $product['price'] && (!$themeData->store_common['disable_checkout_stock_status_name'] || $themeData->store_common['disable_checkout_stock_status_name'] != $product['stock_status']);
    ...
    {
        $theme_templates = $this->engine->getThemeConfig('templates');

        if (!$theme_templates || !in_array('listing_product', $theme_templates)) {
            return;
        }
        ...
        $elements = array('thumb', 'title', 'description', 'price', 'tax', 'rating', 'button_cart', 'button_wishlist', 'button_compare', 'stock_status', 'label_sale', 'label_new');
        ...
        $product_layout = $this->engine->getThemeExtension()->fetchTemplate($this->context->getThemeDir() . '/templates/listing_product.tpl', $view_data);
