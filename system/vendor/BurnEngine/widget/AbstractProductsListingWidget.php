...
abstract class AbstractProductsListingWidget extends TB_Widget
{
    ...
    public function buildStyles(TB_StyleBuilder $styleBuilder)
    {
        ...
        {
            $css = '#' . $this->getDomId() . ' .tb_grid_view:not(.tbHoverReady) ' . str_replace(', ', ', #' . $this->getDomId() . ' .tb_grid_view:not(.tbHoverReady) ', $active_elements) . ' {
              display: none;
            }';
    ...
    public function render(array $view_data = array())
    {
        ...
        */
        ...
        $js = trim(str_replace(
            ...
            array($this->getDomId(), ($parent_id ? 'true' : 'false'), $parent_id, ($this->themeData->optimize_js_load ? 'true' : 'false')),
            $js)
        );
