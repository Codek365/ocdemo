...
class Theme_AlsoBoughtProductsWidget extends AbstractProductsListingWidget
{
    ...
    protected function getProductsListing(array $options = array())
    {
        if (null === $this->products) {
            ...
            $cache_key = 'also_bought_products.' . md5(implode(',', $product_ids)) . '.' . $this->settings['filter_limit'] . '.' . $this->language_code . '.' . $this->engine->getCurrentStoreId() . '.' . $this->getThemeModel()->getCustomerGroupId();
            if (!$also_bought_product_ids = $this->engine->getCacheVar($cache_key)) {
                $also_bought_product_ids = $this->getModel('products')->getAlsoBoughtProductIds($product_ids, $options);
