...
class Theme_MenuWidget extends TB_Widget
{
    ...
    public function onEdit(array &$settings)
    {
        if (!empty($settings['separator_image'])  && file_exists(DIR_IMAGE . $settings['separator_image'])) {
            $settings['separator_image_preview'] = $this->getOcModel('tool/image')->resize($settings['separator_image'], 100, 100);
        } else {
    ...
    public function buildStyles(TB_StyleBuilder $styleBuilder)
    {
        ...
        $menu_font_size   = $this->settings['box_styles']['font'][$this->language_code]['level_1']['size'];
        $level_2_size     = $this->settings['box_styles']['font'][$this->language_code]['level_2']['size'];
        $level_2_height   = $this->settings['box_styles']['font'][$this->language_code]['level_2']['line-height'];
        ...
        if ($this->settings['orientation'] == 'vertical' && $this->settings['menu_height'] > 0) {
            ...
            $css .= '}';
            $css .= '#' . $this->getDomId() . ' > nav > .nav > li:not(:last-child) > a ~ ul:not(.dropdown-menu) {';
    ...
    public function getDefaultBoxColors()
    {
        $default_colors = array(
            ...
            'menu_level_1'  => array(
                ...
                'icons' => array(
                    'label'       => 'Icons',
                    'elements'    => 'nav > ul > li > a:not(:hover) > .tb_text > .tb_icon',
                    'property'    => 'color',
                    'color'       => '#333333',
                    'important'   => 0,
                    'force_print' => 0,
            ...
            'menu_level_2'  => array(
                ...
                'icons' => array(
                    'label'       => 'Icons',
                    'elements'    => '
                        nav > ul > li:not(.dropdown) > ul > li > a:not(:hover) > .tb_text > .tb_icon,
                        nav > ul > li.tb_hidden_menu > ul > li > a:not(:hover) > .tb_text > .tb_icon
                    ',
                    'property'    => 'color',
                    'color'       => '#333333',
                    'important'   => 0,
