...
class Theme_Admin_LayoutBuilderController extends TB_AdminController
{
    ...
    protected function populateAreaBuilderData($area_name, $area_type, $area_id)
    {
        ...
        $this->data['area_type'] = $area_type;
        $this->data['area_id']   = $area_id;
        $this->data['can_add_rows'] = $this->user->hasPermission('modify', (version_compare(VERSION, '2.2.0.0', '>=') ? 'theme/' : 'module/') . $this->context->getBasename());
    ...
    {
        if (!$this->user->hasPermission('modify', (version_compare(VERSION, '2.2.0.0', '>=') ? 'theme/' : 'module/') . $this->context->getBasename())) {
