...
class Theme_Admin_ModuleController extends Controller
{
    public function index()
    {
        ...
        if (($request->server['REQUEST_METHOD'] == 'POST') && !$this->user->hasPermission('modify', (version_compare(VERSION, '2.2.0.0', '>=') ? 'theme/' : 'module/') . TB_BASENAME) && !$this->ignoredPostPermissions()) {
        ...
        if (!version_compare(VERSION, '2.2.0.0', '>=') && !in_array(TB_BASENAME, $this->registry->get('tbManager')->getThemeModel()->getInstalledOcModules())) {
            return new Action('error/permission');
        } else
        if (version_compare(VERSION, '2.2.0.0', '>=') && !in_array(TB_BASENAME, $this->registry->get('tbManager')->getOcModel('extension/extension')->getInstalled('theme'))) {
            return new Action('error/permission');
        }
