...
class Theme_Admin_Extension extends TB_AdminExtension
{
    ...
    function afterRouting()
    {
        ...
        foreach ($this->themeData->manufacturers as $manufacturer) {
            $js_manufacturers[] = array(
                ...
                'name' => addslashes($manufacturer['name'])
            );
        }
    ...
    protected function checkInstallRequirements()
    {
        ...
        if ($this->manager->gteOc2() && !is_writable(DIR_MODIFICATION)) {
            $errors[] = 'The folder <strong>' . DIR_MODIFICATION . 'modification</strong> needs to be writable for OpenCart to work. Please change its permissions to 777';
        }
