...
class Theme_Catalog_Extension extends TB_CatalogExtension
{
    ...
    public function viewOutput(TB_ViewSlotEvent $event)
    {
        ...
        $styles = $this->manager->getOcDocument()->getStyles();
    ...
    protected function buildCssCacheHash()
    {
        ...
        foreach ($this->request_areas_settings as $area => $key) {
    ...
    protected function buildResourceCacheHash($type)
    {
        ...
        if (!isset($file_hash[$type])) {
            $hash = array();
            $list = $type == 'css' ? $this->request_areas_settings : $this->themeData->request_areas;
            foreach ($list as $area => $key) {
                $hash[] = $area . '_' . str_replace('/', '_', $key);
            }
