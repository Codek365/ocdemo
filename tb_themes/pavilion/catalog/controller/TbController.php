...
class Theme_Catalog_TbController extends TB_CatalogController
{
    ...
    public function getProductPrice()
    {
        ...
        $recurring_id = (int) $this->getArrayKey($this->manager->gteOc2() ? 'recurring_id' : 'profile_id', $this->request->post, 0);
        ...
        $result = array(
            'price'    => $this->themeData->priceFormat($this->manager->getOcCurrency()->format($price)),
            'special'  => $this->themeData->priceFormat($this->manager->getOcCurrency()->format($cart->getTotal())),
            'subtotal' => $this->manager->getOcCurrency()->format($cart->getSubTotal())
        );
    ...
    protected function saveCartData()
    {
        if (version_compare(VERSION, '2.1.0.0') >= 0) {
            ...
            foreach ($this->manager->getOcCart()->getProducts() as $product) {
                $cart_data[] = array(
    ...
    protected function restoreCartData()
    {
        ...
        if (version_compare(VERSION, '2.1.0.0') >= 0) {
            foreach ($this->cart_data as $data) {
                $this->manager->getOcCart()->add($data['product_id'], $data['quantity'], $data['option'], $data['recurring_id']);
            }
        } else {
