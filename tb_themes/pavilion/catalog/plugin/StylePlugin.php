...
class Theme_Catalog_StylePlugin extends TB_ExtensionPlugin
{
    ...
    public function buildStyles(TB_StyleBuilder $styleBuilder)
    {
        ...
        $css_fonts_url = $this->context->getThemeConfig('css_fonts_url');
        if (empty($css_fonts_url) || $css_fonts_url == 'auto_relative') {
            $css_replace = array('../font/' => '../../../catalog/view/theme/' . $this->context->getBasename() . '/font/');
        } else
        if ($css_fonts_url == 'auto_full') {
            $css_replace = array('../font/' => '{path}font/');
        } else {
            $css_replace = array('../font/' => str_replace('{{basename}}', $this->context->getBasename(), $css_fonts_url));
        }
